...
#create function to generate json web token
...
#create function to verify a token is present and valid
...
#verify the owner of the drone is the one trying to access the drone
def verify_owner(current_user_token):
    owner = Drone.query.filter_by(user_id = current_user_token.token).first()
    print(current_user_token)
    print(owner)
    if owner == None:
        return jsonify({'message':'You dont have any drones created! Create One!!'})
